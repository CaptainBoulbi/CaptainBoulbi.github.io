...
function Thunder(options) {
  options = options || {};
  this.lifespan = options.lifespan || Math.round(Math.random() * 2 + 10);
...
function Spark(options) {
  ...
  this.lifespan = options.lifespan || Math.round(Math.random() * 4 + 20);
  this.maxlife = this.lifespan;
...
function Text(options) {
  ...
  buffer.font = `${this.size}px MonoSpace`;
...
(function () {
  ...
  //
  text = new Text({
    copy: "En cours de developpement",
  });
